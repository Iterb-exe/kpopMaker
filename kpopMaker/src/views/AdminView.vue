...
<template>
  <div class="min-h-screen bg-gray-900 p-8 font-sans">
    ...
    <div v-if="!isAuthorized" class="max-w-md mx-auto bg-gray-800 p-8 rounded-xl shadow-2xl border border-gray-700 mt-20">
      <h2 class="text-2xl font-black text-white mb-6 text-center uppercase tracking-widest">Wymagana Autoryzacja</h2>
      ...
      <input 
        ...
        class="w-full bg-gray-900 border border-gray-600 rounded px-4 py-3 text-white mb-4 focus:outline-none focus:border-emerald-500 transition-colors"
        @keyup.enter="fetchAllTournaments"
      >
      <button 
        @click="fetchAllTournaments"
        class="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3 rounded transition-colors shadow-lg"
      >
    ...
    <div v-else>
      <div class="flex justify-between items-end mb-8">
        <h1 class="text-3xl font-black text-white uppercase tracking-widest">Panel Administracyjny</h1>
        <button 
          @click="handleLogout"
          class="text-sm font-bold text-gray-400 hover:text-red-400 transition-colors"
        >
          Wyloguj się
        </button>
      </div>

      <h2 class="text-2xl font-bold text-emerald-400 mb-4">Oczekujące na decyzję ({{ pendingTours.length }})</h2>
        ...
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="bg-gray-950 text-gray-400 text-xs uppercase tracking-wider border-b border-gray-700">
              <th class="px-6 py-5 font-bold">Gracz</th>
              <th class="px-6 py-5 font-bold">Data Rozegrania</th>
              <th class="px-6 py-5 font-bold text-center w-32">Akceptuj</th>
              <th class="px-6 py-5 font-bold text-center w-32">Usuń</th>
            </tr>
          </thead>
          <tbody>
      ...
      <h2 class="text-2xl font-bold text-gray-300 mb-4 mt-12">Zatwierdzona historia gry ({{ approvedTours.length }})</h2>
      <div class="overflow-hidden bg-gray-800 rounded-xl shadow-xl border border-gray-700 opacity-80 hover:opacity-100 transition-opacity">
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="bg-gray-950 text-gray-400 text-xs uppercase tracking-wider border-b border-gray-700">
              <th class="px-6 py-5 font-bold">Gracz</th>
