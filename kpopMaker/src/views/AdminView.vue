...
const fetchAllTournaments = async () => {
  ...
  try {
    ...
    if (response.status === 401) {
      ...
      throw new Error(t('admin.unauthorizedAccess'))
    }
...
<template>
  <div class="min-h-screen bg-gray-900 p-4 md:p-8 font-sans">
    ...
    <div v-if="!isAuthorized" class="max-w-md mx-auto bg-gray-800 p-6 md:p-8 rounded-xl shadow-2xl border border-gray-700 mt-10 md:mt-20">
      <h2 class="text-xl md:text-2xl font-black text-white mb-6 text-center uppercase tracking-widest">{{ $t('admin.requiredAuthorization') }}</h2>
      ...
      <input v-model="adminPassword" type="password" autocomplete="new-password" :placeholder="$t('admin.passwordPlaceholder')" class="w-full bg-gray-900 border border-gray-600 rounded px-4 py-3 text-white mb-4 focus:outline-none focus:border-emerald-500 transition-colors" @keyup.enter="fetchAllTournaments">
      <button @click="fetchAllTournaments" class="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3 rounded transition-colors shadow-lg">
        <span v-if="isLoading">{{ $t('admin.loading') }}</span><span v-else>{{ $t('admin.login') }}</span>
      </button>
    </div>

    <div v-else>
      <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-8">
        <h1 class="text-2xl md:text-3xl font-black text-white uppercase tracking-widest">{{ $t('admin.panelTitle') }}</h1>
        <button @click="handleLogout" class="text-sm font-bold text-gray-400 hover:text-red-400 transition-colors bg-gray-800 md:bg-transparent px-4 py-2 md:p-0 rounded-lg md:rounded-none">{{ $t('admin.logout') }}</button>
      </div>

      <h2 class="text-xl md:text-2xl font-bold text-emerald-400 mb-4">{{ $t('admin.pendingTitle', { count: pendingTours.length }) }}</h2>
      <div v-if="pendingTours.length === 0" class="bg-gray-800 border border-gray-700 p-6 rounded-xl shadow-xl text-center mb-12">
        <span class="text-emerald-500 font-bold text-lg">{{ $t('admin.noPendingResults') }}</span>
      </div>
        ...
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="bg-gray-950 text-gray-400 text-xs uppercase tracking-wider border-b border-gray-700">
              <th class="px-4 md:px-6 py-4 font-bold">{{ $t('admin.player') }}</th>
              <th class="hidden sm:table-cell px-4 md:px-6 py-4 font-bold">{{ $t('admin.date') }}</th>
              <th class="px-4 md:px-6 py-4 font-bold text-right">{{ $t('admin.actions') }}</th>
            </tr>
          </thead>
          <tbody>
      ...
      <h2 class="text-xl md:text-2xl font-bold text-gray-300 mb-4 mt-8 md:mt-12">{{ $t('admin.approvedHistory', { count: approvedTours.length }) }}</h2>
